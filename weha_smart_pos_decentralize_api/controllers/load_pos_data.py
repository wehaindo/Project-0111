...
class PosCategory(http.Controller):    
    ...
    @validate_token
    @http.route('/pos/decentralize/load_pos_data', type="http", auth="none", methods=["POST"], csrf=False)
    def load_pos_data(self, **kwargs):        
        try:
            data = json.loads(request.httprequest.data)
            if 'pos_session_id' in data and 'model' in data:
                pos_session_id = request.env['pos.session'].sudo().browse(data['pos_session_id'])
                if pos_session_id:
                    loaded_data = pos_session_id._load_model(data['model'])
                    data_return =  {
                        "err": False,
                        "message": "",
                        "data": loaded_data     
